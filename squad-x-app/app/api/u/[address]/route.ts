...
export async function POST(request: Request, { params }: { params: { address: string } }) {
  ...
  const { username, fullName, id, account, picture } = await request.json()
  const user = await db.select().from(users).where(eq(users.address, address))

  if (user.length === 0) {
    const user = await db.insert(users).values({ address, username, fullName, id, account, picture }).returning()
